...
struct Node {
    ...
    Node(int patientID, string patientName, string doctorName ,string appointmentTime): patientID(patientID), patientName(patientName), doctorName(doctorName), appointmentTime(appointmentTime), prev(nullptr), next(nullptr) {};
};
...
class HospitalManagementSystem {
    ...
    void addPatient(int patientID, string patientName, string doctorName ,string appointmentTime) {
        Node* newNode = new Node(patientID, patientName, doctorName, appointmentTime);
        if(head == nullptr) {
            head = tail = newNode;
        } else {
            head->prev = newNode;
    ...
    void displayAllAppointments() {
        ...
        while (current != nullptr) {
            cout << "Patient ID: " << current->patientID << ", Doctor: " << current->doctorName
